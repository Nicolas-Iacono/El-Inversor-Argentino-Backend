...
public class ArticleController {
    ...
    @CrossOrigin(origins = "http://localhost:3000")
    @PreAuthorize("permitAll()")
    public ResponseEntity<List<ArticleAuthorSalidaDto>> getArticlesByAuthorUsername(@PathVariable String username) {
        List<ArticleAuthorSalidaDto> articles = articleService.buscarArticulosPorAuthor(username);
    ...
    public ResponseEntity<String> eliminarArticulo(@PathVariable Long id) {
        try {
            ...
            return new ResponseEntity<>("Error al eliminar el artículo: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    @CrossOrigin(origins = "http://localhost:3000")
